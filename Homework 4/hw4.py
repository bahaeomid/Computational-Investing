...
dict_keys=['Date','Sym','Order','Qty']
my_dict_buy=dict(zip(dict_keys,[None]*len(dict_keys)))
my_dict_sell=my_dict_buy.copy()
orders=pd.DataFrame(columns=dict_keys)
...
for sym in syms:
	for i in range(1,len(opentimes)):
		...
		if price_sym_today<8.0 and price_sym_yes>=8.0:
			events[sym][i]=1
			my_dict_buy[dict_keys[0]]=events.index[i]
			my_dict_buy[dict_keys[1]]=sym
			my_dict_buy[dict_keys[2]]='Buy'
			my_dict_buy[dict_keys[3]]=100
			if events.index[i] <= events.index[len(events.index)-6]:
				my_dict_sell[dict_keys[0]]=events.index[i+5]
			else: my_dict_sell[dict_keys[0]]=events.index[len(events.index)-1]
			my_dict_sell[dict_keys[1]]=sym
			my_dict_sell[dict_keys[2]]='Sell'
			my_dict_sell[dict_keys[3]]=100		
			orders=orders.append(my_dict_buy,ignore_index=True)
			orders=orders.append(my_dict_sell,ignore_index=True)
...
print '\nThe orders data frame has been exported to a csv file and saved into your working directory.'
