...
start_date=parser.parse(start_date)-dt.timedelta(hours=16)
...
for i in range(1,len(values)):
	ret=(values['Value'][i]-values['Value'][i-1])/values['Value'][i-1]
	if ret<-0.3 or ret>0.45:
			ind.append(values.index[i])

			
values=values.drop(values.index[ind[0]:ind[1]])
spx=spx.drop(spx.index[ind[0]:ind[1]])
